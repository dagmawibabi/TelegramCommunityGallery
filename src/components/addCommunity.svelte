                    ...
                    {#each tags as tag}
                        ...
                        <div class="p-1 lowercase">
                            {#if radioValue == "channel"}
                                {#if selectedTags.indexOf(tag) == -1}
                                    <span on:click={(e) => addRemoveTag(tag)} class="rounded-full border border-zinc-700 w-fit pb-1 px-3 hover:bg-emerald-400 hover:text-black"> {tag} </span>
                                {:else}
                                    <span on:click={(e) => addRemoveTag(tag)} class="rounded-full border border-zinc-700 w-fit pb-1 px-3 bg-emerald-400 text-black"> {tag} </span>
                                {/if}
                            {:else}
                                {#if selectedTags.indexOf(tag) == -1}
                                    <span  on:click={(e) => addRemoveTag(tag)} class="rounded-full border border-zinc-700 w-fit pb-1 px-3 hover:bg-cyan-400 hover:text-black"> {tag} </span>
                                {:else}
                                    <span  on:click={(e) => addRemoveTag(tag)} class="rounded-full border border-zinc-700 w-fit pb-1 px-3 bg-cyan-400 text-black"> {tag} </span>
                                {/if}
                            {/if}
                        </div>
                    {/each}
